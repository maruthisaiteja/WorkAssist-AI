...
export default function FacultyAssignedOverview() {
  ...
  useEffect(() => {
    setMounted(true);
    fetch('/api/faculty/assigned-dashboard')
      .then(res => res.json())
      .then(d => setData(d));
      
    // Fetch notifications specifically related to tasks they assigned
  ...
  const statsCards = [
    { label: 'Total Tasks Assigned', value: data.stats?.total || 0, icon: CheckCircle2, color: 'text-blue-600', bg: 'bg-blue-50' },
    { label: 'Completed', value: data.stats?.completed || 0, icon: CheckCircle2, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'In Progress', value: data.stats?.inProgress || 0, icon: Clock, color: 'text-blue-500', bg: 'bg-blue-50' },
    { label: 'Pending', value: data.stats?.pending || 0, icon: AlertCircle, color: 'text-amber-500', bg: 'bg-amber-50' },
    { label: 'Overdue', value: data.stats?.overdue || 0, icon: AlertCircle, color: 'text-red-600', bg: 'bg-red-50' },
  ];
  ...
  return (
    <div className="space-y-10 animate-in pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-8 rounded-3xl border border-slate-200 card-shadow">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Tasks Assigned By You</h1>
          <p className="text-slate-500 font-medium mt-1">Monitor the progress of tasks you've delegated to others.</p>
        </div>
        <Link href="/dashboard/faculty/assign" className="flex items-center gap-2 px-8 py-4 bg-blue-600 text-white rounded-2xl hover:bg-blue-700 transition-all shadow-xl shadow-blue-600/30 text-base font-bold">
          <Plus size={20} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-3 bg-white rounded-3xl border border-slate-200 card-shadow overflow-hidden flex flex-col">
          <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/50">
            <h3 className="font-bold text-slate-800 text-lg">Delegated Task Monitor</h3>
          ...
          <div className="overflow-x-auto">
            ...
            {filteredTasks.length === 0 && (
              <div className="py-24 text-center">
                ...
                <p className="text-slate-400 font-bold text-lg">No delegated tasks found</p>
                <p className="text-slate-400 text-sm mt-1">You haven't assigned any tasks yet.</p>
              </div>
            )}
          </div>
        </div>
